...
let rec cgExp (ctxt : context) (Exp {exp_base; ty; _} : H.exp) :
  ...
  | H.IfExp {test; thn; els= Some e} ->
      ...
      (* brancher på resultatet af guarden, og terminerer basic blocken *)
      ...
      let res_b, res_op =
        if ty_to_llty ty == Void then
          ...
          (* kan slettes, laves også i forrige block*)
  ...
  | H.WhileExp {test; body} ->
      ...
      (* add tjek_inst og "læg" resultatet i op *)
      let br = Ll.Cbr (g_cmp_op, loop_lab, merge_lab) in
      (* branch på test til loop eller done *)
